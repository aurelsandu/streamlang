...
// HotPathOp: Map(x*10) → Filter(x%20==0) → Skip(1) → Take(2)
func TestHothPath_Map_Filter_Skip_Take(t *testing.T) {
	...
	// compunere fără Compose4: (op1→op2→op3) → op4
	pipe := stream.Compose(
		stream.Compose3(map10, filter20, skip1), // Op[int,int]
		take2,                                   // Op[int,int]
	)
